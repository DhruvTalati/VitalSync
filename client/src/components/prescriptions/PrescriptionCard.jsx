...
import { Download, Trash2, ClipboardList, StickyNote } from 'lucide-react';

const PrescriptionCard = ({ prescription, patientName, onDownload, onDelete }) => {
  return (
    <div className="card-surface overflow-hidden">
      <div className="bg-gradient-to-r from-primary-600 to-indigo-600 text-white px-6 py-4 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <span className="bg-white/20 text-xs font-bold px-2.5 py-1 rounded-md">Rx #{prescription.rxNumber}</span>
          <p className="font-bold">{prescription.title}</p>
        </div>
        <p className="text-sm text-primary-100">
          {new Date(prescription.issuedDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
        </p>
      </div>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-full bg-primary-600 text-white font-bold flex items-center justify-center">
            {prescription.doctorName?.charAt(0) || 'D'}
          </div>
          <div>
            <p className="font-semibold text-sm text-primary-950">Dr. {prescription.doctorName}</p>
            <p className="text-xs text-slate-500">{prescription.specialization}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button onClick={() => onDownload(prescription, patientName)} className="btn-primary text-sm">
            <Download size={16} /> Download PDF
          </button>
          <button onClick={() => onDelete(prescription._id)} className="bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl flex items-center gap-2 transition-colors">
